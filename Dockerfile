...
RUN apt-get update && apt-get install -y  git build-essential make wget  zlib1g-dev libc6 pigz minia &&\
	pip install snakemake


RUN ln -s $(which minia) /binaries/minia
...
COPY capture_contigs.py /opt/capture_contigs.py
